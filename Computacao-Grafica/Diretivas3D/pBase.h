...
void gMeusDesenhos() {
    ...
    // parte inferior (altura yBaixo)
    glPushMatrix(); glTranslatef(-desloc,  -desloc,  desloc); glutSolidSphere(1.0, 32, 32); glPopMatrix();  // frente esquerda
   	glPushMatrix(); glTranslatef( desloc,  -desloc,  desloc); glutSolidCube(1); glPopMatrix();  // frente direita
	glPushMatrix(); glTranslatef(-desloc,  -desloc,  -desloc); glutSolidTorus (0.3, 0.7, 16, 32); glPopMatrix();  // trás esquerda
    // parte superior (altura yCima)
    glPushMatrix(); glTranslatef(-desloc,  desloc,  desloc); glutSolidCone(0.8, 1.5, 16, 16); glPopMatrix();  // frente esquerda
    glPushMatrix(); glTranslatef( desloc,  desloc,  desloc); glutSolidIcosahedron(); glPopMatrix();  // frente direita
    glPushMatrix(); glTranslatef(-desloc,  desloc,  -desloc); glutSolidDodecahedron(); glPopMatrix();  // trás esquerda
    glPushMatrix(); glTranslatef( desloc,  desloc,  -desloc); glutSolidTeapot(1); glPopMatrix();  // trás direita
    
    glColor3f(0, 1, 0);
    
    // parte inferior (altura yBaixo)
    glPushMatrix(); glTranslatef(-desloc2, -desloc2, desloc2); glutWireSphere(1.0, 32, 32); glPopMatrix();  // frente esquerda
    glPushMatrix(); glTranslatef( desloc2, -desloc2, desloc2); glutWireCube(1); glPopMatrix();  // frente direita
    glPushMatrix(); glTranslatef(-desloc2, -desloc2, -desloc2); glutWireTorus (0.3, 0.7, 16, 32); glPopMatrix();  // trás esquerda
    // parte superior (altura yCima)
    glPushMatrix(); glTranslatef(-desloc2, desloc2, desloc2); glutWireCone(0.8, 1.5, 16, 16); glPopMatrix();  // frente esquerda
    glPushMatrix(); glTranslatef( desloc2, desloc2, desloc2); glutWireIcosahedron(); glPopMatrix();  // frente direita
	glPushMatrix(); glTranslatef(-desloc2, desloc2, -desloc2); glutWireDodecahedron(); glPopMatrix();  // trás esquerda
	glPushMatrix(); glTranslatef( desloc2, desloc2, -desloc2); glutWireTeapot(1); glPopMatrix();  // trás direita
    
    glPushMatrix();
		glBegin (GL_LINES);
			glColor3f (1, 0, 0);
			glVertex3f (-15, 0.0, 0.0); glVertex3f (15, 0.0, 0.0);
			glVertex3f (0.0, -15, 0.0); glVertex3f (0.0, 15, 0.0);
			glColor3f (0, 0, 0);
			glVertex3f (0.0, 0.0, -15); glVertex3f (0.0, 0.0, 15);
		glEnd ();
	glPopMatrix();
}
